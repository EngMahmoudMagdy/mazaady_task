...
import com.magdy.core.models.CategoryProperty
import com.magdy.core.models.ChildrenItem
import com.magdy.core.services.NetworkResult
import com.magdy.mazaadytask.R
import com.magdy.mazaadytask.databinding.ActivityFirstPageBinding
            ...
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                ...
                launch {
                    mViewModel.catPropertiesState.collect {
                        when (it) {
                            is NetworkResult.Success -> {
                                Log.d("NetworkResult","Success catPropertiesState")
                                dismissLoading()
    ...
    private fun initViews() = with(binding) {
        mainCat.setOnClickListener {
            CategoryBottomSheetFragment(mainCatList.map { it.name ?: "" }) {
                //On Item from BottomSheet selected:
        ...
        subCat.setOnFocusChangeListener { view, hasFocus ->
                if (hasFocus) {
                    subCat.performClick()
                }
            }
